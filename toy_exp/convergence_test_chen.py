...
from layers import ChenLinear, Lorentz
...
def run_experiment(
    ...
    x = torch.tensor([time_input.item(), space_input.item()] + [0.0] * (dim - 2), dtype=torch.float64)
...
def save_results_to_txt(results: Dict[int, List[int]], filename: str):
    ...
    with open(filename, 'w') as f:
        ...
        for dist, iters in sorted(results.items()):
            ...
            f.write(f"{dist:<12} | {euclidean_height:<20} | {mean_iters:7.1f} ± {std_iters:5.1f} | {status:<15}\n")
...
def main():
    ...
    for dist, iters in sorted(results.items()):
        ...
        print(f"{dist:<12} | {euclidean_height:<20} | {mean_iters:7.1f} ± {std_iters:5.1f} | {status:<15}")
    ...
    if max_converged_distance:
        ...
        print(f"   (Euclidean height ≈ e^{max_converged_distance} ≈ {np.exp(max_converged_distance):.2e})")
    else:
